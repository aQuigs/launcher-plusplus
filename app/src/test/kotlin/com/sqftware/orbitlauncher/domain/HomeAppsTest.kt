...
class HomeAppsTest {
    ...
    fun `removing an app takes a folder it empties with it`() {
        ...
        assertEquals(HomeApps(ring = Ring(listOf(folder(mail, maps))), dock = homeApps.dock), homeApps.remove(HomePlace.Folder(0), clock))
        assertEquals(HomeApps(ring = Ring(listOf(folder(clock), folder(maps))), dock = homeApps.dock), homeApps.remove(HomePlace.Folder(1), mail))
        assertEquals(HomeApps(ring = homeApps.ring), homeApps.remove(HomePlace.Dock, clock))
    }
    ...
    fun `an app folded onto a ring app makes a folder of the two in that slot and leaves where it came from`() {
        ...
        assertEquals(
            ...
            homeApps.move(maps, HomePlace.Dock, Landing.Into(shownMail)),
        )
        assertEquals(HomeApps(ring = Ring(listOf(folder(mail, clock))), dock = homeApps.dock), homeApps.move(clock, HomePlace.Ring, Landing.Into(shownMail)))
        assertEquals(
            HomeApps(ring = Ring(listOf(folder(clock, music), RingSlot.App(mail.key))), dock = homeApps.dock),
            homeApps.move(music, null, Landing.Into(shownClock)),
        )
    }
    ...
    fun `an app folded into a folder joins it at the end once`() {
        ...
        assertEquals(Ring(listOf(folder(clock, mail, maps))), homeApps.move(maps, HomePlace.Ring, Landing.Into(work)).ring)
        assertEquals(HomeApps(ring = homeApps.ring), homeApps.move(mail, HomePlace.Dock, Landing.Into(work)))
        assertEquals(homeApps, homeApps.move(clock, HomePlace.Folder(0), Landing.Into(work)))
    }
    ...
    fun `an app out of a folder takes the folder it empties with it and lands by the slots as they were`() {
        ...
        assertEquals(
            ...
            homeApps.move(clock, HomePlace.Folder(0), Landing.OnRing(games, ReorderMode.Insert)).ring,
        )
        assertEquals(Ring(listOf(RingSlot.App(mail.key), folder(maps, clock))), homeApps.move(clock, HomePlace.Folder(0), Landing.Into(games)).ring)
    }
    ...
    fun `the ring and the dock trade apps, inserted or swapped`() {
        ...
        assertEquals(
            ...
            homeApps.move(maps, HomePlace.Dock, Landing.OnRing(shownMail, ReorderMode.Insert)),
        )
        assertEquals(
            HomeApps(ring = ringOf(clock, maps), dock = Favourites(listOf(mail.key, music.key))),
            homeApps.move(maps, HomePlace.Dock, Landing.OnRing(shownMail, ReorderMode.Swap)),
        )
        assertEquals(
            HomeApps(ring = ringOf(mail), dock = Favourites(listOf(maps.key, clock.key, music.key))),
            homeApps.move(clock, HomePlace.Ring, Landing.InDock(music, ReorderMode.Insert)),
        )
        assertEquals(
            HomeApps(ring = ringOf(mail), dock = Favourites(listOf(maps.key, music.key, clock.key))),
            homeApps.move(clock, HomePlace.Ring, Landing.InDock(null, ReorderMode.Swap)),
        )
    }
    ...
    fun `an app landing in a place that has it moves there rather than doubling`() {
        ...
        assertEquals(
            ...
            homeApps.move(maps, HomePlace.Dock, Landing.OnRing(shownClock, ReorderMode.Insert)),
        )
        ...
        assertEquals(
            ...
            homeApps.move(maps, HomePlace.Dock, Landing.OnRing(work, ReorderMode.Swap)),
        )
        assertEquals(
            HomeApps(ring = Ring(listOf(folder(mail), RingSlot.App(maps.key))), dock = Favourites(listOf(maps.key, clock.key))),
            homeApps.move(clock, HomePlace.Ring, Landing.InDock(null, ReorderMode.Insert)),
        )
    }
