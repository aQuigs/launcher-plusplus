...
class AppSearchTest {
    private val apps = listOf(app("Gmail"), app("Google Maps"), app("Mail"), app("Éclair"), app("7zip"))
    ...
    fun `words starting with the query come before labels merely containing it`() {
        assertEquals(listOf("Google Maps", "Mail", "Gmail"), labels("ma"))
    }
    ...
    fun `a digit before the query is not a word start`() {
        assertEquals(listOf("7zip"), labels("zip"))
        assertEquals(listOf("7zip"), labels("7z"))
    }
