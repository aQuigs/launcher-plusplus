...
class SystemWallClock(private val context: Context) : WallClock {
    ...
    override fun faces(): Flow<ClockFace> = callbackFlow {
    ...
    override fun openCalendar() = startOrLog(TAG, "the calendar") {
        val today = ContentUris.appendId(CalendarContract.CONTENT_URI.buildUpon().appendPath("time"), System.currentTimeMillis())
        context.startActivity(Intent(Intent.ACTION_VIEW, today.build()).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
    }
}
