...
import com.aquigs.launcherplusplus.domain.decodeWidgetPage
import com.aquigs.launcherplusplus.domain.encode

/** A widget pick the system has not answered yet: the id allocated for it and the rows the page had room for. */
data class WidgetPick(val id: Int, val pageRows: Int)
...
class SharedPreferencesWidgetPageStore(context: Context) : WidgetPageStore {
    ...
    override fun loadPick(): WidgetPick? {
        ...
        return if (fields.size == 2 && null !in fields) WidgetPick(fields[0]!!, fields[1]!!) else null
    }

    override fun savePick(pick: WidgetPick?) = prefs.edit {
        if (pick == null) remove(PICK_KEY) else putString(PICK_KEY, "${pick.id}$FIELD${pick.pageRows}")
    }
