...
<!-- Before and after screenshots, or a recording, for any user-visible change; write N/A otherwise.
     Capture with scripts/screenshot.sh or scripts/record.sh, publish with scripts/pr-media.sh <file> <caption>..., and paste the printed table as is
     (fixed-width cells, two per row, so every screenshot comes out the same size whatever its caption). -->
...
- [ ] Before/after screenshots or a recording are attached for user-visible changes (or marked N/A)
