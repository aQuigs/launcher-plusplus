...
- Sessions share the emulator. Wrap device work in `L=/tmp/launcher-plusplus-emulator.lock; until mkdir $L 2>/dev/null; do sleep 10; done; trap 'rmdir $L' EXIT; …`; build first, never hold it while building, debugging or waiting.
