...
- Parallel sessions share one emulator, guarded by the lock directory `/tmp/launcher-plusplus-emulator.lock`. Hold it only while on the device (install, `run.sh`, connected tests, screenshots, recordings, `adb` input), never while building, reading code, debugging a failure or waiting on reviewers. Build the APKs first (`./gradlew assembleDebug assembleDebugAndroidTest`), then take and release the lock in the same command: `L=/tmp/launcher-plusplus-emulator.lock; until mkdir $L 2>/dev/null; do sleep 10; done; trap 'rmdir $L' EXIT; …`. Pin `ANDROID_SERIAL` to the emulator, because a phone may be attached too. A connected run uninstalls the app and wipes its preferences.
